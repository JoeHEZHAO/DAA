...
def visualize_G_ind(nets, N, real_sample, mode, merge=None, show_dataset=True, kde=False, dim1=0, dim2=1, alpha=0.1):

    '''
    visualize generator for chosen mode:
        Args: real_sample: real posterior distribution for certain mode
                mode: chose wich mode to use;
    '''
    ...
    out_data = nets.G(in_data, mode, merge)
    out_data = out_data.data.cpu().numpy()
    # data = nets.dataset.data
    plot_outputdata(real_sample, dim1=dim1, dim2=dim2, marker='kx', alpha=alpha)
    plot_outputdata(out_data, dim1=dim1, dim2=dim2, marker='.', alpha=alpha)
    plt.show()
    return real_sample, out_data

def visualize_G_separate(nets, N, real_sample, mask,  show_dataset=True, kde=False, dim1=0, dim2=1, alpha=0.1):
    '''
    visualize generator for only one mode:
        Args: real_sample: real posterior distribution for certain mode
              mask: mask for certain mode
